...
export function extractDifferences(content: string) {
  ...
  for (const line of lines) {
    ...
    if (clean === clean.toUpperCase() && clean.length < 50) continue;
    
    // Skip conversational or intro/outro lines
    const lower = clean.toLowerCase();
    if (
      lower.includes('comparison summary') ||
    ...
    if (
      ...
      if (colonIndex > 5 && colonIndex < 40) {
        ...
        if (lower.includes('payment')) section = 'Payment Terms';
        else if (lower.includes('termination')) section = 'Termination Clause';
        else if (lower.includes('confidentiality') || lower.includes('nda')) section = 'Confidentiality';
        else if (lower.includes('indemnity')) section = 'Indemnity / Liability';
        else if (lower.includes('duration') || lower.includes('term')) section = 'Duration / Term';
      }
...
export function buildFormValidationResults(formType: string, userInputs: any) {
  ...
  return fields
    .filter((field: string) => labels[field] !== undefined)
    .map((field: string) => {
