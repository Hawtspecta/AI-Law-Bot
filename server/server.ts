...
import { ragPipeline } from "./backend/ragServices";
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    const ragResponse = await ragPipeline(message, language || "en");
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.status === 'ok' && data.articles && data.articles.length > 0) {
            let news = data.articles.map((article: any, index: number) => ({
              id: index + 1,
            ...
            if (topic && topic !== 'general' && topicKeywords[topic]) {
              ...
              news = news.filter((article: any) => {
                const title = article.title.toLowerCase();
                const summary = article.summary.toLowerCase();
                return keywords.some((kw: string) => title.includes(kw) || summary.includes(kw));
              });
            }
            ...
            if (news.length < limit) {
              ...
              for (const fallback of topicArticles) {
                ...
                if (!news.some((n: any) => n.title.toLowerCase() === fallback.title.toLowerCase())) {
                  news.push(fallback);
                }
              }
            }
